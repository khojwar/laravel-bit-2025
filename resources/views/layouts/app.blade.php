...
<body class="bg-gray dark:bg-gray-900">
            ...
            <ul class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                @if (!Auth::check()) 
                    ...
                    @if (request()->routeIs('register.form'))
                            <!-- If on Register page, show Login link -->
                            <li>
                                <a href="{{ route('login.form') }}" 
                                class="block  px-3 text-gray-900 hover:text-blue-700 dark:text-white">
                                Login
                                </a>
                            </li>
                        @elseif (request()->routeIs('login.form'))
                            <!-- If on Login page, show Register link -->
                            <li>
                        ...
                        @else
                            <!-- If on other pages, you can show both (optional) -->
                            <li>
                                <a href="{{ route('login.form') }}" class="block  px-3 dark:text-white">Login</a>
                            </li>
                            <li>
                                <a href="{{ route('register.form') }}" class="block  px-3 dark:text-white">Register</a>
                            </li>
                        @endif
                ...
                <li>
                    ...
                    @if (Auth::check())
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-1 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Logout</button>
                        </form>
                    @endif
                </li>
            </ul>
